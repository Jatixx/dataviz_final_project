...
Taken together, the five visualizations tell a coherent story about how
popular summer music changed between 1958 and 2017: summer hits got
louder, more beat-driven, and less acoustic, with a few interesting
wrinkles.

The line chart (Visualization 1) shows the broad arc. Acousticness drops
sharply from the 1960s onward, while energy and danceability rise. One
possible explanation is the shift from acoustic, guitar-driven pop
toward electric and electronically produced music, though the data alone
cannot confirm the cause. The most surprising result is valence: it
peaks around the 1970s and then declines, so recent summer hits are more
intense and danceable than ever yet score lower on positivity than songs
from forty years earlier. This ran against my initial expectation that
more energetic music would also sound happier.

The artist bar chart (Visualization 2) shows that a small group of
artists, led by Rihanna and Katy Perry, account for a large share of
summer chart appearances. This is consistent with the idea that
commercial success is not purely random, although the chart only counts
appearances and cannot speak to why.

The boxplot (Visualization 3) adds nuance: median danceability rises,
but the spread also narrows in recent decades. The 1960s show a wider
mix of dance rhythms, whereas by the 2010s the range tightens. One
possible reading is that audiences or chart curators have converged on a
narrower idea of what summer music should sound like.

The scatterplots (Visualizations 4 and 5) confirm that danceability and
energy are positively correlated, and that higher-valence songs tend to
cluster where both are high. The interactive version lets the reader
hover over any point to read the song title, artist, and exact values,
so a surprising outlier can be identified by name rather than guessed
at. That is something a static image cannot offer.
