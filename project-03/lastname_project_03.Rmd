...
ggplot(weather_tpa, aes(x = max_temp, fill = factor(month))) +
  # stat = "bin" ist der Standard für Histogramme, r_width sorgt für die Abstände
  geom_histogram(color = "white", binwidth = 2.5, show.legend = FALSE) +
  
  # 3 Reihen, 4 Spalten (passend zur Grafik)
  facet_wrap(~ month, nrow = 3, ncol = 4) +
  
  # Die "Viridis"-Farbpalette sorgt für den exakten Farbverlauf von Lila zu Gelb
...
```{r}
ggplot(weather_tpa, aes(x = max_temp, fill = factor(month))) +
  geom_density(color = "white", alpha = 0.7, show.legend = FALSE) +
  scale_fill_viridis_d(option = "viridis") + 
  theme_minimal() +
  labs(
    title = "Monatliche Temperaturprofile (Geglättet)",
